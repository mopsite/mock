...
::: warning 注意
Random.image() 用于生成高度自定义的图片地址，一般情况下，应该使用更简单的 [Radom.dataImage()](#dataimage)。
:::
...
- **size**：可选。指示图片的宽高，格式化为 `宽x高`。默认从下面的数组中随机读取一个：
...
- **format**：可选。指示图片的格式。包括：`png`、`gif`、`jpg`，默认值为 `png`。
...
![image](http://dummyimage.com/200x100/FF6600/FFF.jpg&text=Mock.js)
...
::: warning 注意
图片的背景色是随机的，取值范围参考 <http://brandcolors.net>。
:::
